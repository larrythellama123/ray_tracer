...
int main() {
    ...
    std::cout<<bvh.max.x<<bvh.max.y<<bvh.max.z<<"OG BVH MAX"<<std::endl;
    std::cout<<bvh.min.x<<bvh.min.y<<bvh.min.z<<"OG BVH MIN"<<std::endl;
...
std::vector<BVHNode> SAH_BVH(std::vector<Triangle>& triangle_arr, BVH bvh){
    ...
    int parent_node_num = 0;
    bvh.offset = triangle_arr.size();
    ...
    while(!BVH_list.empty()){
        bvh = BVH_list[0];
        BVH_list.pop_front();
        // std::cout << bvh.max.x <<"HELLO"<< bvh.max.y<<" "<< bvh.max.z<<std::endl;
        // std::cout << bvh.min.x <<"HELLO"<< bvh.min.y<<" "<< bvh.min.z<<std::endl;
        ...
        float step_Y = (bvh.max.y -bvh.min.y)/(num_buckets); 
        bool flag[3];

        for(int i{1};  i<num_buckets+1; i++){
            float boundary_Z = bvh.min.z + step_Z*(i);
            float boundary_X = bvh.min.x +  step_X*(i);
            float boundary_Y = bvh.min.y +  step_Y*(i);
        ...
        std::cout << best_bvh_pair[0].offset<<" "<<best_bvh_pair[0].primitive_index<< "ererere "<< best_bvh_pair[0].count<<std::endl;
...
std::array<BVH, 2>  SplittingAndCost(glm::vec4 bvh_tmp_1_max, glm::vec4 bvh_tmp_2_min, float boundary, float& best_cost_function, BVH& bvh, std::array<BVH, 2>  bvh_pair, float parent_area, bool* flag, std::vector<std::pair<Triangle, glm::vec3>>& data){
        ...
        std::vector<std::pair<Triangle, glm::vec3>>::iterator partition_point;
